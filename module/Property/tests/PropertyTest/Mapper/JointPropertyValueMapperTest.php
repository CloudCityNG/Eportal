...
class JointPropertyValueMapperTest extends AbstractPropertyMapperTest {
    ...
    /**
     *
     * @var JointPropertyValueMapper
     */
    protected $mapper;

    /**
     *
     * @var JointPropertyValueHydrator
     */
    protected $hydrator;
    ...
    protected function setUp() {
        parent::setUp();
        $this->mapper = new JointPropertyValueMapper();
        $this->mapper->setEntityPrototype(new JointPropertyValue());
    ...
    public function testFindById() {
        ...
        $entity = new JointPropertyValue($id, $fpv, $spv);
    ...
    public function testFindByFirstPropertyValue() {
        ...
        $results = $this->mapper->findByFirstPropertyValue($fpvId);
        $realOutcome = [];
    ...
    public function testFindFirstPropertyValue(){
        ...
        $results = $this->mapper->findFirstPropertyValue($property, $spv);
        $realOutcome = [];
        foreach($results as $result){
//            \Zend\Debug\Debug::dump($result);
    ...
    public function testFindSecondPropertyValue() {
        ...
        $results = $this->mapper->findSecondPropertyValue($property, $fpv);
        $realOutcome = [];
        foreach($results as $result){
            $realOutcome[] = $result->getId();
        }
    ...
    public function testHasPropertyValue() {
        ...
        $this->mapper->setHydrator(new ClassMethods())
                ->setEntityPrototype(new JpvTestModel());
        $result = $this->mapper->hasJointPropertyValue($jpv[0], $jpv[1]);
        $this->assertTrue($result);
        $result = $this->mapper->hasJointPropertyValue(rand($count+1, $count+10), rand($count+11, $count+21));
    ...
    public function testFindJointPropertyValue(){
        ...
        $result = $this->mapper->findJointPropertyValue($jpv[0], $jpv[1]);
